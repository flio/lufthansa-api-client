...
module Lufthansa
  class Resource
    ...
    class << self
      ...
      def from_response response, *response_key
      ...
      def from_json json, response_key
        model = nil
        raw = JSON.parse(json)
        # manage case with 2 level deep key
        if response_key.is_a? Array
          raw = raw.dig(*response_key)
          response_key = response_key.last
        end
        response_key ? from_parsed_json(raw, response_key) : raw
      end
      ...
      def string_to_model(model_string)
        ...
        begin
          ...
          Lufthansa.const_get(class_name)
        rescue NameError => e
          nil
        end
      end
      ...
      def string_to_model_with_caching(model_string)
        @@string_to_model_cache_lock.synchronize do
          ...
          model = Lufthansa::CUSTOM_MODELS[model_string] if model.nil?
    ...
    def attributes= attributes = {}
      ...
      attributes.each_pair { |k, v|
        ...
        value = Lufthansa::Resource.from_parsed_json(v, k)
